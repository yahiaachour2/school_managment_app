...
export default function Admin() {
    ...
    useEffect(() => {
      fetchData('ADMIN'); // Fetch students by default
    }, [page, limit]);
    ...
    const handleDelete = async (userId: string) => {
      if (userId) {
        try {
          ...
          // fetchData('ADMIN'); // Fetch students after deletion
          toast.current?.show({ severity: 'info', summary: 'Confirmed', detail: 'Admin deleted', life: 3000 });
        } catch (error) {
          console.error('Error deleting admin:', error);
          setError(`Error deleting admin with ID ${userId}: ${JSON.stringify(error, null, 2)}`);
        }
      }
    };
    ...
    const handleClick = (userId: string) => {
      navigate(`/updateadmin/${userId}`);
    };
  
    const handleClickShow = (userId: string) => {
      navigate(`/showadmin/${userId}`);
    };
    ...
    return (
      <div className="card rounded-md m-6">
        ...
        <div className='flex justify-between pr-10'>
          ...
          <Link to="/createadmin">
            <Button label="Create Admin" className="m-3 p-button-success" />
          </Link>
        </div>
